...
export default function SearchMedicalRecords({ onRecordSelect, userRole }: SearchMedicalRecordsProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/medical-records/search?${params}`, {
        headers: {
          ...
          <div className="space-y-4 p-4 border rounded-lg bg-gray-50">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm text-gray-500">
                {records.length} kết quả tìm thấy
              </span>
            </div>
          </div>
        )}
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">
                          {userRole === 'doctor' ? (record.patientId?.fullName || 'Chưa cập nhật') : (record.doctorId?.fullName || 'Chưa cập nhật')}
                        </span>
