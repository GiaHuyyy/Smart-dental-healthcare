...
export default function ImageAnalysisDisplay({
  ...
  const isCloudinaryError =
    analysisResult.error?.includes("Cloudinary") ||
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-red-700">
              <p className="mb-2">{analysisResult.error || "Dịch vụ lưu trữ ảnh chưa được cấu hình đúng cách."}</p>
              <div className="bg-white p-3 rounded border border-red-200">
                <p className="font-medium text-red-800 mb-2">Cách khắc phục:</p>
                <ol className="list-decimal list-inside space-y-1 text-xs">
                  <li>
                    Tạo tài khoản Cloudinary tại{" "}
                    <a
        ...
        <div className="space-y-3">
          {analysisResult.richContent?.analysis && (
            <div>
              ...
              <p className="text-sm text-gray-700 bg-blue-50 p-3 rounded">{analysisResult.richContent.analysis}</p>
            </div>
          )}
