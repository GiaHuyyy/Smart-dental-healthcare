...
export default function DoctorSchedule() {
  ...
  const [suggestions, setSuggestions] = useState({
    ...
    diagnosisMedicationMap: {} as Record<string, Array<{ name: string, dosage: string, frequency: string, duration: string, instructions: string }>>,
    treatmentMedicationMap: {} as Record<string, Array<{ name: string, dosage: string, frequency: string, duration: string, instructions: string }>>
  });
  ...
  function filterSuggestions(field: string, input: string) {
    ...
    switch (field) {
      ...
      case 'diagnosis':
        const filteredDiagnoses = suggestions.diagnoses.filter(item =>
          item.toLowerCase().includes(lowerInput)
        );
        setFilteredSuggestions(prev => ({
          ...prev,
      ...
      case 'treatmentPlan':
        const filteredPlans = suggestions.treatmentPlans.filter(item =>
          item.toLowerCase().includes(lowerInput)
        );
        setFilteredSuggestions(prev => ({
          ...prev,
      ...
      case 'Enter':
        ...
        if (selectedIndex >= 0 && selectedIndex < suggestions_array.length) {
          ...
          if (field.startsWith('medication-') && groupIndex !== undefined) {
            ...
            const isDuplicate = treatmentForm.diagnosisGroups[groupIndex].treatmentPlans.some((existingPlan, idx) =>
              idx !== planIndex && existingPlan.toLowerCase().trim() === selectedSuggestion.toLowerCase().trim() && existingPlan.trim() !== ''
            );
  ...
  function updateMedication(index: number, field: 'name' | 'dosage' | 'frequency' | 'instructions', value: string) {
    ...
    if (field === 'name' && value.length > 0) {
      ...
      const filteredMeds = suggestions.medications.filter(item =>
        item.toLowerCase().includes(lowerInput)
      );
      setFilteredSuggestions(prev => ({
        ...prev,
  ...
  function updateDiagnosis(groupIndex: number, value: string) {
    ...
    if (value.trim() && value !== previousDiagnosis && value.length >= 3) {
      ...
      const isCompleteDiagnosis = suggestions.diagnoses.some(d =>
  ...
  function isTreatmentPlanDuplicate(groupIndex: number, newPlan: string): boolean {
    ...
    return group.treatmentPlans.some(existingPlan =>
      existingPlan.toLowerCase().trim() === normalizedNewPlan && existingPlan.trim() !== ''
    );
  }
  ...
  function autoAddMedicationsForDiagnosis(diagnosis: string) {
    ...
    let medicationsToAdd: Array<{ name: string, dosage: string, frequency: string, duration: string, instructions: string }> = [];
    ...
    if (medicationsToAdd.length > 0) {
      setTreatmentForm(prev => {
        ...
        const newMedications = medicationsToAdd.filter(med =>
          !existingMedNames.includes(med.name.toLowerCase().trim())
        );
  ...
  function updateTreatmentPlan(groupIndex: number, planIndex: number, value: string) {
    ...
    if (value.trim() && value !== previousPlan && value.length >= 3) {
      ...
      const isCompletePlan = suggestions.treatmentPlans.some(p =>
    ...
    if (value.length > 0) {
      ...
      const lowerInput = value.toLowerCase();
      let filteredPlans = relevantPlans.filter(item =>
        item.toLowerCase().includes(lowerInput)
      );

      // Remove plans that already exist in this diagnosis group (excluding current input)
      filteredPlans = filteredPlans.filter(plan => {
        const isDuplicate = currentGroup.treatmentPlans.some((existingPlan, idx) =>
          idx !== planIndex && existingPlan.toLowerCase().trim() === plan.toLowerCase().trim() && existingPlan.trim() !== ''
        );
        return !isDuplicate;
      });
  ...
  async function submitTreatment() {
    ...
    try {
      ...
      const medicalRecordData = {
        ...
        diagnosisGroups: treatmentForm.diagnosisGroups
          .filter(group => group.diagnosis.trim() !== ''), // Full structure
        ...
        medications: treatmentForm.medications
          ...
          .map(med => med.name), // Simple array of strings for compatibility
        detailedMedications: treatmentForm.medications
          .filter(med => med.name.trim() !== ''), // Full objects with dosage, frequency etc.
                  ...
                  <div key={id} className="bg-gray-50 rounded-lg p-4 border">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      {apptStatus !== 'in-progress' && apptStatus !== 'completed' && (
                        <button disabled={!!apptLoading[id]} onClick={() => updateStatus(id, 'confirm')} className="text-green-600 hover:text-green-800 text-sm">
                          {apptLoading[id] ? '...' : '✅ Xác nhận'}
                        </button>
                      )}
                      {apptStatus === 'completed' ? (
                        <button disabled={!!apptLoading[id]} onClick={() => startTreatment(appointment)} className="text-purple-600 hover:text-purple-800 text-sm font-medium">
                          {apptLoading[id] ? '...' : '🔍 Xem lại điều trị'}
                        </button>
                      ) : (
                        <button disabled={!!apptLoading[id]} onClick={() => startTreatment(appointment)} className="text-purple-600 hover:text-purple-800 text-sm">
                          {apptLoading[id] ? '...' : '🏥 Điều trị'}
                        </button>
                      )}
                      {apptStatus !== 'completed' && (
                        <>
                          <button disabled={!!apptLoading[id]} onClick={() => confirmAndCancel(id)} className="text-gray-600 hover:text-gray-800 text-sm">
                            {apptLoading[id] ? '...' : '❌ Hủy'}
                          </button>
                          <button disabled={!!apptLoading[id]} onClick={() => rescheduleAppointment(id)} className="text-yellow-600 hover:text-yellow-800 text-sm">
                            📅 Đổi lịch
                          </button>
                        </>
                      )}
          ...
          <div className="fixed inset-0 bg-opacity-15 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-gray-300">
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="font-medium">Họ tên:</span> {currentTreatmentAppointment.patient?.fullName}</p>
                      <p><span className="font-medium">Điện thoại:</span> {currentTreatmentAppointment.patient?.phone}</p>
                      <p><span className="font-medium">Email:</span> {currentTreatmentAppointment.patient?.email}</p>
                      <p><span className="font-medium">Ngày khám:</span> {new Date(currentTreatmentAppointment.appointmentDate).toLocaleDateString('vi-VN')}</p>
                      <p><span className="font-medium">Giờ khám:</span> {currentTreatmentAppointment.startTime}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      <div className="flex justify-between items-center mb-2">
                        <label className="block text-sm font-medium">Lý do khám <span className="text-red-500">*</span></label>
                        <button
                          type="button"
                      ...
                      <input
                        ...
                        placeholder={treatmentForm.chiefComplaints.length === 0 ? "Nhập lý do khám và nhấn Enter hoặc dấu phẩy..." : "Thêm lý do khám khác..."}
                      />
                        ...
                        <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-md shadow-lg z-10 max-h-40 overflow-y-auto">
                          {filteredSuggestions.chiefComplaints.map((suggestion, index) => (
                            <div
                              ...
                              className={`px-3 py-2 cursor-pointer text-sm ${index === selectedSuggestionIndex.chiefComplaint
                ...
                <div className="mt-6">
                  <div className="flex justify-between items-center mb-2">
                    <label className="block text-sm font-medium">Chẩn đoán & Kế hoạch điều trị <span className="text-red-500">*</span></label>
                    <button
                      type="button"
                    ...
                    {treatmentForm.diagnosisGroups.map((group, groupIndex) => (
                      <div key={groupIndex} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          ...
                          <div>
                            <div className="flex justify-between items-center mb-2">
                              <label className="block text-sm font-medium text-gray-700">Chẩn đoán {groupIndex + 1}</label>
                            ...
                            <div className="flex gap-2 items-start">
                              <div className="flex-1 relative">
                                ...
                                />
                                {showSuggestions[`diagnosis-${groupIndex}` as keyof typeof showSuggestions] && filteredSuggestions.diagnoses.length > 0 && (
                                  <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-md shadow-lg z-20 max-h-40 overflow-y-auto">
                                    {filteredSuggestions.diagnoses.map((suggestion, suggIndex) => (
                                      <div
                                        ...
                                        className={`px-3 py-2 cursor-pointer text-sm ${suggIndex === selectedSuggestionIndex.diagnosis
                                            ? 'bg-blue-100 text-blue-800'
                                            : 'hover:bg-gray-100'
                                          }`}
                                      >
                                ...
                                <div key={planIndex} className="flex gap-2">
                                  <div className="flex-1 relative">
                                    <input
                                      ...
                                      onKeyDown={(e) => handleKeyDown(e, `treatmentPlan-${groupIndex}-${planIndex}`, groupIndex, planIndex)}
                                      className="w-full border border-gray-300 rounded-md p-2 text-sm"
                                      placeholder={`Kế hoạch ${planIndex + 1}`}
                                    />
                                    {showSuggestions[`treatmentPlan-${groupIndex}-${planIndex}` as keyof typeof showSuggestions] && filteredSuggestions.treatmentPlans.length > 0 && (
                                      <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-md shadow-lg z-20 max-h-40 overflow-y-auto">
                                        {filteredSuggestions.treatmentPlans.map((suggestion, suggIndex) => (
                                          <div
                                            ...
                                            onClick={() => {
                                              // Check for duplicates before adding
                                              const isDuplicate = treatmentForm.diagnosisGroups[groupIndex].treatmentPlans.some((existingPlan, idx) =>
                                                idx !== planIndex && existingPlan.toLowerCase().trim() === suggestion.toLowerCase().trim() && existingPlan.trim() !== ''
                                              );

                                              if (isDuplicate) {
                                                addToast(`Kế hoạch điều trị "${suggestion}" đã tồn tại trong chẩn đoán này!`, 'error');
                                                return;
                                              }
                                            ...
                                            className={`px-3 py-2 cursor-pointer text-sm ${suggIndex === selectedSuggestionIndex.treatmentPlan
                                                ? 'bg-blue-100 text-blue-800'
                                                : 'hover:bg-gray-100'
                                              }`}
                                          >
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {treatmentForm.medications.map((medication, index) => (
                        <div key={`medication-${index}-${medication.name || 'empty'}`} className="border border-gray-200 rounded-md p-3 relative">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div className="relative">
                              ...
                              {showSuggestions[`medication-${index}` as keyof typeof showSuggestions] && filteredSuggestions.medications.length > 0 && (
                                <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-md shadow-lg z-20 max-h-40 overflow-y-auto">
                                  {filteredSuggestions.medications.map((suggestion, suggIndex) => (
                                    <div
                                      ...
                                      className={`px-3 py-2 cursor-pointer text-sm ${suggIndex === selectedSuggestionIndex.medication
                                          ? 'bg-blue-100 text-blue-800'
                                          : 'hover:bg-gray-100'
                                        }`}
                                    >
