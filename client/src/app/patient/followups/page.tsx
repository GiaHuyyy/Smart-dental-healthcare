...
interface FollowUpDoctorSummary {
  _id: string;
  fullName: string;
...
const isFollowUpDoctorSummary = (value: unknown): value is FollowUpDoctorSummary => {
  if (!value || typeof value !== "object") {
    return false;
  }
...
const isFollowUpByDoctor = (value: unknown): value is FollowUpByDoctor => {
  if (!value || typeof value !== "object") {
    return false;
  }
  const candidate = value as Record<string, unknown>;
  ...
  return isFollowUpDoctorSummary(candidate.doctor) && hasValidLastAppointment && isFollowUpSchedule(candidate.followUps);
};
...
const fetchFollowUps = async (patientId: string): Promise<FollowUpByDoctor[]> => {
  const response = await fetch(`/api/appointments/followup/patient/${patientId}`, {
    cache: "no-store",
  });
  if (!response.ok) {
  ...
  return payload.filter(isFollowUpByDoctor);
};
    ...
    <Suspense
      fallback={
        <div className="min-h-screen p-6">
          <div className="max-w-4xl mx-auto healthcare-card p-4">Đang tải lịch tái khám...</div>
        </div>
      }
    >
      <PatientFollowupsContent />
    </Suspense>
  );
}

function PatientFollowupsContent() {
  ...
  const bookWithDoctor = (doctorId: string, doctorName?: string | null) => {
        ...
        <div className="healthcare-card p-4">
          {loading ? (
            ...
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.doctor._id} className="p-3 border rounded-md flex items-center justify-between">
                  <div>
                    <div className="font-medium">{item.doctor.fullName}</div>
                    <div className="text-sm text-gray-600">Chuyên khoa: {item.doctor.specialty || "N/A"}</div>
                    <div className="text-sm text-gray-600">Lần khám gần nhất: {formatDate(item.lastAppointmentDate)}</div>
                  </div>
                  <div className="text-right text-sm flex flex-col items-end gap-2">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <button
                        type="button"
                        onClick={() => bookWithDoctor(item.doctor._id, item.doctor.fullName)}
