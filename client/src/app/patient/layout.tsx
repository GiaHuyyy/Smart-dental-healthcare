...
export default function PatientLayout({ children }: { children: React.ReactNode }) {
  ...
  function parseRescheduleMessage(msg: string | undefined) {
    ...
    const re = /Giờ cũ:\s*([0-9T:\-\.Z]+)\s+([0-9]{1,2}:[0-9]{2})\D+Giờ mới:\s*([0-9T:\-\.Z]+)\s+([0-9]{1,2}:[0-9]{2})/i;
  ...
  useEffect(() => {
    ...
    async function fetchNotifications() {
      try {
        const res = await sendRequest<any>({ method: 'GET', url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/notifications/user/${userId}` });
        const list = res?.data || res || [];
        if (!Array.isArray(list)) return;
        // Filter to appointment-related notifications (reschedules) and pick the newest one only
        const apptNotifs = list.filter((n: any) => (n && (n.type === 'appointment' || n.type === 'appointment:update' || n.type === 'appointment_change')) && (n.refModel === 'Appointment' || !n.refModel));
        ...
        if (!seenRef.current.has(id) && !isRead) {
          ...
          try {
            ...
            if (id && typeof id === 'string') {
              sendRequest<any>({ method: 'PATCH', url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/notifications/${id}/read` }).catch(() => {});
            }
          } catch (e) {
        ...
        if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
          ...
          bc.onmessage = (ev) => {
            try {
              ...
              if (!(payload.type === 'appointment' || payload.type === 'appointment:update' || payload.type === 'appointment_change')) return;
              ...
              if (id && typeof id === 'string') sendRequest<any>({ method: 'PATCH', url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/notifications/${id}/read` }).catch(() => {});
            } catch (e) {
    ...
    return () => { mounted = false; clearInterval(t); if (bc) bc.close(); };
  }, [session]);
  ...
  useEffect(() => {
    if (broadcastModal) {
      ...
      // focus close button
      setTimeout(() => {
        try { modalCloseRef.current?.focus(); } catch (e) {}
      }, 0);
    } else {
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-gray-600">Bác sĩ đã cập nhật thời gian khám của bạn — vui lòng kiểm tra chi tiết bên dưới.</p>
              </div>
            </div>
                ...
                return (
                  <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 rounded-lg border border-red-100 bg-red-50 shadow-sm">
                      ...
                      <div className="mt-1 text-sm text-red-800 font-semibold">{formatDateTime(parsed.oldDate, parsed.oldTime)}</div>
                    </div>
                    <div className="p-4 rounded-lg border border-emerald-100 bg-emerald-50 shadow-md">
                      ...
                      <div className="mt-1 text-lg font-bold text-emerald-900">{formatDateTime(parsed.newDate, parsed.newTime)}</div>
                    </div>
                    <div className="md:col-span-2 text-sm text-gray-600 mt-2">Nếu giờ mới không phù hợp, vui lòng liên hệ phòng khám hoặc nhấn "Liên hệ" để được trợ giúp.</div>
                  </div>
                );
              }
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              <button onClick={() => { window.location.href = '/patient/settings'; }} className="px-4 py-2 border rounded hover:bg-gray-50">Liên hệ</button>
              <button ref={modalCloseRef} onClick={closeBroadcastModal} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Đóng</button>
            </div>
          </div>
        </div>
      ) : null}
